...
## Introduction
Welcome to the Corchea repository! A simple plug and play low-cost and open-source hardware that support a paper based microfluidic system for biological applications. The device use negativepressure to generate constant flow of liquid without pumps. The liquid flow through wax channels in paper that can be printed or hand-drawn with crayons for rapid prototyping. The device can be easily plugged into [Fluopi](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0187163) for fluorescent image acquisition. It can be used for bacterial culture,directed communication and pulse induction.
